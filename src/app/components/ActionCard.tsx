...
import { GripVertical, X, Edit2, Link2, Plus } from 'lucide-react';
...
export function ActionCard({ action, onUpdate, onDelete, allActions = [], dependencies = [], onAddDependency, onRemoveDependency }: ActionCardProps) {
  ...
  const getTagColor = (value: 'H' | 'M' | 'L', type: string) => {
  ...
  const currentDependencies = dependencies.filter(dep => dep.toId === action.id);
  const availableActions = allActions.filter(a => a.id !== action.id);
          ...
          {isEditing ? (
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-4 gap-1">
                {(['urgency', 'risk', 'acceptance', 'effort'] as const).map((field) => (
                  <select
                    key={field}
                    value={editData[field]}
                    onChange={(e) => setEditData({ ...editData, [field]: e.target.value as 'H' | 'M' | 'L' })}
              ...
              {onAddDependency && onRemoveDependency && (
                <div className="border-t border-gray-200 pt-2">
                  <div className="flex items-center justify-between mb-1">
                    <label className="text-xs font-medium text-gray-600" style={{ fontFamily: 'var(--font-sans)' }}>
                      Dependencies
                    </label>
                    <button
                  ...
                  {currentDependencies.length > 0 && (
                    <div className="space-y-1 mb-2">
                      {currentDependencies.map((dep) => {
                        const fromAction = allActions.find(a => a.id === dep.fromId);
                        return fromAction ? (
                          <div key={dep.id} className="flex items-center justify-between gap-2 bg-gray-50 rounded px-2 py-1">
                            <div className="flex items-center gap-1.5 min-w-0 flex-1">
                              <Link2 size={10} className="text-gray-400 flex-shrink-0" />
                              <span className="text-[10px] text-gray-700 truncate" style={{ fontFamily: 'var(--font-sans)' }}>
                                {fromAction.title}
                              </span>
                              <span className={`text-[9px] px-1 py-0.5 rounded flex-shrink-0 ${dep.type === 'cross-team' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>
                                {dep.type === 'cross-team' ? 'Cross' : 'Intra'}
                              </span>
                            </div>
                            <button
                              onClick={() => handleRemoveDependency(dep.fromId)}
                  ...
                  {showDependencyPicker && (
                    <div className="border border-gray-300 rounded bg-white max-h-32 overflow-y-auto">
                      {availableActions.filter(a => !currentDependencies.some(dep => dep.fromId === a.id)).length === 0 ? (
                        <div className="px-2 py-1.5 text-xs text-gray-500 text-center">
                          No more tasks available
                        </div>
                      ) : (
                        availableActions
                          .filter(a => !currentDependencies.some(dep => dep.fromId === a.id))
                          .map((a) => (
                            <button
                              key={a.id}
                            ...
                            >
                              <div className="font-medium text-gray-700 truncate" style={{ fontFamily: 'var(--font-sans)' }}>
                                {a.title}
                              </div>
                              <div className="text-[10px] text-gray-500 flex items-center gap-1">
                                <span>{a.team}</span>
                                <span>•</span>
                                <span className="uppercase">{a.timeframe}</span>
                              </div>
                            </button>
                          ))
                      )}
            ...
            <>
              <div className="flex items-start justify-between gap-2 mb-2">
                <h4 className="text-sm leading-tight" style={{ fontFamily: 'var(--font-sans)', fontWeight: 500 }}>
                  {action.title}
                </h4>
                <div className="flex gap-1">
                  <button
              ...
              <div className="flex flex-wrap gap-1 mb-2">
                ...
                <span className={`px-1.5 py-0.5 rounded text-[10px] ${getTagColor(action.acceptance, 'acceptance')}`} style={{ fontFamily: 'var(--font-mono)' }}>
                  A:{action.acceptance}
                </span>
